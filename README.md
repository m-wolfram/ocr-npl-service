...
#### 2. Поиск объектов.
    ...
    Например алгоритмом: https://blog.roboflow.com/training-a-yolov3-object-detection-model-with-a-custom-dataset/.
    
    Разметка прилагается к заданию.
    Её можно преобразовать под нужный формат и использовать в качестве входных данных для обучения модели вместе с картинками.
    Типы объектов указать sign и logo.
    Далее оформить в виде отдельного модуля аналогично другим модулям.
...
#### ***. Реализация проекта.
    ...
    Общая структура обработки запросов следующая:
        ...
        2. Тело запроса направляется в отдельный файл обработки запроса определенного типа в utils.
        3. В файле происходит подготовка полученных данных для обработки модулями и/или пайплайнами, формируется ответ.
