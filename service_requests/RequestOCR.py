...
class RequestOCR():
    ...
    def save_image_on_server(self, save_dir):
        """
        Method saves image from received in ocr request json.
    ...
    def prepare_response(self, recognition_result):
        """Method preparies server image recognition response."""
    ...
    def process_ocr_request(self):
        """
        Method processes recognition request.
        Required request format:
        {
            "filename": name of the image to be recognized,
            "file_b64": encoded image in base64
        ...
        response = self.prepare_response(recognition_result)
